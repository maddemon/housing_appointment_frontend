...
export default class BatchEditModal extends Component {
  ...
  handleSubmit = async data => {
    data.BatchHouses = data.houseId.map(houseId => {
      return { houseId: houseId };
    });
    data.appointmentBeginTime = data.appointmentTime[0].format(
      "YYYY-MM-DD 00:00:00"
    );
    data.appointmentEndTime = data.appointmentTime[1].format(
      "YYYY-MM-DD 00:00:00"
    );
  ...
  getFormItems = () => {
    ...
    return [
      { name: "id", defaultValue: model.id || "0", type: "hidden" },
      {
        ...
        render: (
          <Select
            ...
            mode="multiple"
            placeholder="请选择楼盘"
            onChange={this.handleChooseHouses}
          >
            {houses.map(item => (
      ...
      },
      {
        title: "选房地址",
        name: "chooseAddress",
        defaultValue: model.chooseAddress,
        rules: [{ required: true, message: "请填写选房地址" }]
      },
      {
        title: "预约时段",
        name: "appointmentTime",
        defaultValue: [
          moment(model.appointmentBeginTime),
          moment(model.appointmentEndTime)
        ],
        type: "rangedate",
