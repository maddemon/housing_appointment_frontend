    ...
    getMenuItems = (identity) => {
        ...
        if (identity) {
            ...
            if (identity.role === 'user') {
                result.push(<Menu.Item key="edit_password"> <Icon type="key" /> 修改密码 </Menu.Item>)
                result.push(<Menu.Item key="/user/logout"> <Icon type="poweroff" /> 退出 </Menu.Item>)
            } else {
                result.push(<SubMenu key="user_menu" title={<><Icon type="user" /> {identity.name} <Icon type="caret-down" /></>}>
            ...
            <>
                ...
                <Row key="menu">
                    ...
                    <Col xs={0} sm={0} md={19} log={19} xl={19} xxl={20}>
                        <Menu onClick={this.handleMenuClick} mode="horizontal" selectedKeys={this.state.current} theme={theme} style={{ lineHeight: '64px' }}>
                            {this.getMenuItems(identity)}
                        </Menu>
                    </Col>
                </Row>
