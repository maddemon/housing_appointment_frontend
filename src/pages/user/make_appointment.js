...
import { Row, Col, Button, PageHeader, Alert, Icon, Card, Tag, message, Modal } from 'antd'
...
const SelectedBatchControl = props => {
    ...
    return (
        ...
        >
            ...
            <p>房屋地址：{model.address}</p>
            <p>选房时间：{model.chooseTime}</p>
            <p>选房地点：{model.chooseAddress}</p>
            <p>预约开始时间：{new Date(model.appointmentTimeStart).toLocaleString()}</p>
            <p>预约截止时间：{new Date(model.appointmentTimeEnd).toLocaleString()}</p>
        </Card>
    )
}
...
class BatchItemControl extends Component {
    ...
    render() {
        ...
        return (
            <Card hoverable={false}
                title={model.name}
                extra={<Button type="primary" onClick={this.handleClick}><Icon type="plus" />我要预约</Button>}
            >
                <p>房屋数量：{model.houseNumber}</p>
                <p>房屋地址：{model.address}</p>
                <p>选房时间：{model.chooseTime}</p>
